...
const App: React.FC = () => {
  ...
  const [showDebugConnection, setShowDebugConnection] = useState(false);
  const {
  ...
  if (!authed) {
    if (showDebugConnection) {
      return (
        <DebugConnection 
          onBack={() => setShowDebugConnection(false)}
        />
      );
    }
    if (showDatabaseTest) {
      return (
        <DatabaseTest 
          onBack={() => setShowDatabaseTest(false)}
        />
      );
    }
    if (showRegister) {
